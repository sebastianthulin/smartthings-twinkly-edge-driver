-- Exponential Value Curve Test
-- Test different exponential powers for value curve to optimize HSV->RGB->HSV round-trip accuracy

local ColorConverter = require "src/services/color_converter"
local config = require "src/twinkly/config"

local function test_exp_power(power)
  config.color.value_curve = "exp"
  config.color.value_exp_power = power
  local converter = ColorConverter:new()
  print("\nTesting exp power:", power)
...
local best_power, best_error = nil, math.huge
for power = 0.5, 2.0, 0.1 do
  local v2 = test_exp_power(power)
  local v = math.pow(0.5, power)
...
print("\nBest exp power for value curve:", best_power, "(error:", best_error, ")")
