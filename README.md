...
The driver API has been exercised directly against a Twinkly TWKP200RGB on firmware 2.9.1 at `192.168.87.32`: brightness readback matched 1, 50 and 100 percent; color, built-in effects and saved movies selected successfully; and a stale authentication token recovered after another client logged in. The new version is installed on an Aeotec hub. SmartThings reports seven custom and five built-in Effect choices for device Moln; hub commands selected built-in effect ID 1 and saved movie ID 0 (Carnival) on the physical light. The light was restored to effect ID 0 and 2 percent brightness after the test. Fresh pairing, mobile app rendering, and visual LED output still need checking. Use `smartthings edge:drivers:logcat` to inspect hub-side failures.
